...
export default function PlansPage() {
  ...
  const handleShow = useCallback(async()=>{
    ...
    try {
      ...
      const response = await axios.get("http://localhost:3000/auth/plans_read" , {
        headers: {
  ...
  const handleSubmit = async (e:any , _id:any)=>{
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3000/auth/plans_update/${_id}` , submitForm , {
        headers: {
          Authorization: `Bearer ${token}`, 
        },
      })
  ...
  return (
    <div>
      ...
      />
      <PageBreadcrumb pageTitle="Plans Page" />
