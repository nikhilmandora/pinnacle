...
export default function LocationMapPage() {
  ...
  const handleShowData = useCallback(async()=>{
    ...
    try{
      ...
      const response = await axios.get("http://localhost:3000/auth/location_map_read" , {
        headers: {
  ...
  const handleUpdate = async ( e:any , _id:any )=> {
    ...
    try{
      ...
      const response = await axios.put(`http://localhost:3000/auth/location_map_update/${_id}` , submitForm , {
        headers: {
          Authorization: `Bearer ${token}`, 
        },
      });
  ...
  return (
    <div>
      ...
      />
      <PageBreadcrumb pageTitle="Location Map Page" />
