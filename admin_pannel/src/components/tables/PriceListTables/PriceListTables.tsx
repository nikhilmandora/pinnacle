...
import PriceListForm from "../../form/PriceListForm/PriceListForm";
...
export default function PriceListTable() {
  ...
  const handleShow = useCallback(async()=>{
    ...
    try {
      ...
      const response = await axios.get("http://localhost:3000/auth/price_list_read" , {
        headers: {
  ...
  const handleDelete = async (_id:any)=>{
    ...
    try {
      const token = sessionStorage.getItem("authToken");
      const response = await axios.delete(`http://localhost:3000/auth/price_list_delete/${_id}` , {
        headers: {
          Authorization: `Bearer ${token}`, 
        },
      })
  ...
  const handleUpdateStatus = async (_id:any , status:boolean)=>{
    ...
    try {
      const token = sessionStorage.getItem("authToken");
      const response = await axios.put(`http://localhost:3000/auth/price_list_update_status/${_id}` , {status: !status} , {
        headers: {
          Authorization: `Bearer ${token}`, 
        },
      })
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {showData?.map((dataForShow:any , index:any)=>(
              <TableRow key={index}>
                <TableCell className="w-[2%] px-4 py-3 text-gray-700 text-start text-theme-sm dark:text-gray-400">
                  {index < 9 ? "0" : ""}{index + 1}
                </TableCell>
                ...
                <TableCell className="w-[5%] px-4 py-3 text-gray-700 text-start text-theme-sm dark:text-gray-400">
                  {dataForShow?.area}
                </TableCell>
                <TableCell className="w-[22%] px-4 py-3 text-gray-700 text-start text-theme-sm dark:text-gray-400">
                  {dataForShow?.description}
                </TableCell>
                <TableCell className="w-[5%] px-4 py-3 text-gray-700 text-start text-theme-sm dark:text-gray-400">
                  {dataForShow?.companyPrice}
                </TableCell>
                <TableCell className="w-[5%] px-4 py-3 text-gray-700 text-start text-theme-sm dark:text-gray-400">
                  {dataForShow?.ourPrice}
                </TableCell>
                <TableCell className="px-4 py-3 text-gray-700 text-start text-theme-sm dark:text-gray-400">
      ...
      <div className={`${formPopUp == true ? "flex" : "hidden"} h-[100vh] bg-black/60 w-full fixed z-[5445156] top-0 left-0 justify-center items-center`}>
        <div className="w-[50%] max-h-[80%] rounded-4xl overflow-y-auto">
          <PriceListForm setFormPopUp={setFormPopUp} type={type} dataForId={dataForId} key={childRef} />
        </div>
      </div>
