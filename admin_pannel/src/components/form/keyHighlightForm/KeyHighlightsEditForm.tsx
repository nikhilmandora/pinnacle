...
export default function KeyHighlightsEditForm({ setFormPopUp , type , dataForID } : { setFormPopUp:any , type:string , dataForID:any }) {
  ...
  const handleSubmit = async ( e:any , forID:any )=> {
    ...
    if(!formData.heading || !formData.description || (type == "add" && !formData.image)){
      setError(true);
      return;
    }
    ...
    try {
      ...
      if(type == "add") {
        ...
        const response = await axios.post("http://localhost:3000/auth/key_highlights_create" , submitData , {
          headers: {
      ...
      if(type == "edit") {
        const token = sessionStorage.getItem("authToken");
        const response = await axios.put(`http://localhost:3000/auth/key_highlights_update/${forID}` , submitData , {
          headers: {
            Authorization: `Bearer ${token}`, 
          },
        })
  ...
  return (
    <ComponentCard title={`${type == "add" ? "Add" : "Edit" } Highlights`} className="relative">
      <div className="space-y-2">
        <div>
          <Label>Heading</Label>
          <Input value={formData.heading} onChange={(e)=>setFormData((prev)=>({...prev , heading: e.target.value}))} type="text" />
        ...
        <div>
          ...
          <Input value={formData.description} onChange={(e)=>setFormData((prev)=>({...prev , description: e.target.value}))} type="text" />
          <div className={`${error == true ? "" : "opacity-0"} font-light text-red-600 text-[12px] `}>
            Please Fill Fields.
          </div>
        </div>
