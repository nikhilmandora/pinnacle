...
export default function PriceListForm({setFormPopUp, type, dataForId} : {setFormPopUp:any, type:string, dataForId:any}) {
  ...
  const handleSubmit = async (e:any , forId:any)=> {
    ...
    if(!formData.heading || !formData.area || !formData.description || !formData.companyPrice || !formData.ourPrice || (type == "add" && !formData.image)) {
      setError(true);
      return;
    }
    ...
    try {
      ...
      submitData.append("companyPrice" , formData.companyPrice);
      submitData.append("ourPrice" , formData.ourPrice);
      if(formData.image){
      ...
      if(type == "add"){
        ...
        const response = await axios.post("http://localhost:3000/auth/price_list_create" , submitData , {
          headers: {
      ...
      if(type == "edit"){
        const token = sessionStorage.getItem("authToken");
        const response = await axios.put(`http://localhost:3000/auth/price_list_update/${forId}` , submitData , {
          headers: {
            Authorization: `Bearer ${token}`, 
          },
        })
  ...
  useEffect(()=>{
    if(dataForId){
      setFormData({
        ...
        area: dataForId.area, 
        description: dataForId.description,
        companyPrice: dataForId.companyPrice,
        ourPrice: dataForId.ourPrice
      })
    }
  ...
  return (
    <ComponentCard title={`${type == "add" ? "Add" : "Edit" } Price List`} className="relative">
      <div className="space-y-2">
        ...
        <div>
          ...
          <Input onChange={(e)=>setFormData((prev:any)=>({...prev , area: e.target.value}))} value={formData?.area} type="text" />
          <div className={`${error == true ? "" : "opacity-0"} font-light text-red-600 text-[12px]`}>
            Please Fill Fields.
          </div>
        </div>
        <div>
          ...
          <Input onChange={(e)=>setFormData((prev:any)=>({...prev , description: e.target.value}))} value={formData?.description} type="text" />
          <div className={`${error == true ? "" : "opacity-0"} font-light text-red-600 text-[12px]`}>
            Please Fill Fields.
          </div>
        </div>
        <div>
          ...
          <Input onChange={(e)=>setFormData((prev:any)=>({...prev , companyPrice: e.target.value}))} value={formData?.companyPrice} type="text" />
          <div className={`${error == true ? "" : "opacity-0"} font-light text-red-600 text-[12px]`}>
            Please Fill Fields.
          </div>
        </div>
        <div>
          ...
          <Input onChange={(e)=>setFormData((prev:any)=>({...prev , ourPrice: e.target.value}))} value={formData?.ourPrice} type="text" />
          <div className={`${error == true ? "" : "opacity-0"} font-light text-red-600 text-[12px]`}>
            Please Fill Fields.
          </div>
        </div>
