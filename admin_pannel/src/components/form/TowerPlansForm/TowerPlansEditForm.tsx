...
export default function TowerPlansEditForm({setFormPopUp, type, dataForId} : {setFormPopUp:any, type:String, dataForId:any}) {
  ...
  const handleSubmit = async (e:any , forId:any)=> {
    ...
    if(!formData.button || (type == "add" && !formData.image)){
      setError(true);
      return
    }
    ...
    try {
      ...
      submitForm.append("button" , formData.button);
      if(formData.image){
      ...
      if(type == "add"){
        ...
        const response = await axios.post("http://localhost:3000/auth/tower_plans_create" , submitForm , {
          headers: {
      ...
      if(type == "edit"){
        const token = sessionStorage.getItem("authToken");
        const response = await axios.put(`http://localhost:3000/auth/tower_plans_update/${forId}` , submitForm , {
          headers: {
            Authorization: `Bearer ${token}`, 
          },
        })
  ...
  return (
    <ComponentCard title={`${type == "add" ? "Add" : "Edit"} Tower Plans`} className="relative">
      <div className="space-y-2">
        <div>
          <Label>Button Name</Label>
          <Input onChange={(e)=>setFormData((prev:any)=>({...prev , button: e.target.value}))} value={formData?.button} type="text" />
